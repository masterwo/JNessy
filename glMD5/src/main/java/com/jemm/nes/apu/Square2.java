...
public class Square2 implements Runnable {
    private final static float SAMPLE_RATE = 44100f;
    protected int currentHI = ApuRegisters.getInstance().SQ2_HI;
    protected int currentLO = ApuRegisters.getInstance().SQ2_LO;
    private byte[] buffer;
    ...
    public void update() {
    	...
    	if(currentHI != ApuRegisters.getInstance().SQ2_HI || currentLO != ApuRegisters.getInstance().SQ2_LO) {
    		double timer = ((ApuRegisters.getInstance().SQ2_HI << 8) + (ApuRegisters.getInstance().SQ2_LO & 0xFF)) & 0xFFFF;
	...
	void realPlayer() {
	    float l_pulsePeriod = ((ApuRegisters.getInstance().SQ2_HI << 8) + (ApuRegisters.getInstance().SQ2_LO & 0xFF)) & 0xFFFF;
